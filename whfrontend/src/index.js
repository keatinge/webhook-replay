                ...
                <TableBody>
                    {props.data.map(r => (
                        <TableRow key={r[props.colKeys[0]] + r[props.colKeys[1]]}>
                            <TableCell style={{whiteSpace: "nowrap"}}>{r[props.colKeys[0]]}</TableCell>
...
function BodyViewer(props) {
    ...
    return (
        <div>
            <Typography variant={"h6"} style={{marginTop: 40}}>{props.title || "Request Body:"}</Typography>
...
function BoldLabel(props) {
    return (
        <Typography variant={"subtitle2"} style={{fontWeight: "700"}}>{props.text}</Typography>
    );
}
...
function ReplayedReq(props) {
    ...
    return (
        <Paper elevation={3} style={{padding: 10, marginBottom: 20}}>
            <Typography variant={"h6"} style={{marginBottom: 10}}>{props.req.meth} {props.replay.loc} <Chip style={{fontWeight: "700"}} color={chip_style} label={chip_text}/></Typography>
            <Divider />
        ...
        <Tooltip arrow classes={classes} title={props.time}>
            {props.milli ?
                <span>
                    ...
                    .
                    <span style={{fontSize: ".9em", opacity: .75, marginRight: 4}}>{parsedTime.format("SSS")}</span>
                    {parsedTime.format("a")}
                </span> : 
                <span>
                    {parsedTime.format("MMM Do h:mma")}
                </span>
            }
        </Tooltip>
    );
...
function ReqsList(props) {
    return (
        <List>
            ...
            {props.reqs.map(r => (
                <ListItem button key={r.id} selected={r.id === props.curReqId} onClick={() => props.setCurReqIdCallback(r.id)}>
                    <Badge badgeContent={r.replays.length} color={"primary"} variant="dot">
                    <ListItemText primary={`${r.meth} ${r.loc}`} secondary={<DynamicRelTime time={r.time}/>}/>
                    </Badge>
                </ListItem>
            ))}
            {props.reqs.length === 0 && <ListItem>
                <Typography variant="body2" color="textSecondary" component="p">
                    You do not have any captured requests. Once you send a request to your custom URL it will appear here.
                </Typography>
            </ListItem>}
        </List>
    );
...
function CurrentReqDisplay(props) {
    ...
    return (
        <div>
            <Card>
            <CardHeader title={`${props.req.meth} ${props.req.loc}`} subheader={<FullTime time={props.req.time}/>} />
            <CardContent>
            ...
            <ReplaysDisplay replayUrl={props.replayUrl} req={props.req} replays={props.req.replays} sendReplayCallback={props.sendReplayCallback}/>
        </div>);
}
    ...
    return (
        <Card style={{marginTop: 20}}>
            <CardHeader 
                ...
                action={<SendReplayButton replayUrl={props.replayUrl} sendReplayCallback={props.sendReplayCallback}/>}/>
            <CardContent>
                {props.replays.length === 0 && <p>You haven't replayed this request yet. When you do, you will see the HTTP responses here.</p>}
                {props.replays.map(r => (
    ...
    return (
        <span>
            ...
            <Dialog open={open} onClose={handleClose}>
                ...
                <DialogContent>
                    <DialogContentText>You will lose all your captured requests and replays</DialogContentText>
                </DialogContent>
                <DialogActions>
...
function Help(props) {
    return (
        <div>
            <Card>
                <CardHeader title={"You haven't received any requests yet!"} subheader={"Send HTTP requests to your custom URL and they will appear here."} />
                <CardContent>
...
`curl -X 'POST' \\
     ...
     ${props.customUrl}`}
                    ...
                    <Typography variant="body2" color="textSecondary" component="p">
                        Alternatively, you can send an HTTP request using your browser by clicking here:
                        <Link href={props.customUrl} target={"_blank"}> {props.customUrl}</Link>
                    </Typography>
                    <Typography style={{marginTop: 15}} variant="body2" color="textSecondary" component="p">
                        After you send a request. You can wait for the page to automatically update (every 10 seconds), or click
                        on the sync button on the top right to update immediately.
                    </Typography>
...
class App extends React.Component {
    ...
    async sendReplay() {
        ...
        if (this.state.replayUrl.length < 3) {
            this.props.enqueueSnackbar("You must configure your replay url before you can send a replay!", {variant: "error", autoHideDuration: 15_000});
            return;
        }
    ...
    async componentDidMount() {
        ...
        const errFunc = e => {
            this.props.enqueueSnackbar(e, {variant: "error", autoHideDuration: 15_000});
        };
            ...
            <div className={classes.root}>
                ...
                <AppBar position={"static"} className={classes.appBar}>
                    <Toolbar>
                        ...
                        <Link style={{marginRight: 20}} color={"inherit"} href={"https://github.com/keatinge/webhook-replay"} target={"_blank"}><GitHub/></Link>
                        <Button color={"inherit"} onClick={() => this.updateRequests({notify: true})}>Sync <Sync/></Button>
                        <ResetButton registerCallback={this.register.bind(this)}/>
                    </Toolbar>
                </AppBar>
                <Drawer className={classes.drawer} variant="permanent" classes={{paper: classes.drawerPaper}} anchor="left">
                    <ReqsList reqs={this.state.reqs} setCurReqIdCallback={this.setCurReqId.bind(this)} curReqId={this.state.curReqId}/> 
                </Drawer>
                <main className={classes.content}>
                    <Typography variant={"h5"}>Custom URL: <code>{customUrl}</code></Typography>
                    <div className={classes.vertCentered}>
                        <Typography variant={"h5"} style={{display: "inline-block", paddingRight: 10}}>Replay URL: </Typography>
                        <TextField style={{flexGrow: 1}} inputProps={{style: {fontSize: "1.2rem"}}} placeholder={"https://your-server.com/your-endpoint"} onChange={e => this.setState({replayUrl: e.target.value})}/>
                    </div>
                    {currentReq !== undefined && <CurrentReqDisplay replayUrl={this.state.replayUrl} req={currentReq} sendReplayCallback={this.sendReplay.bind(this)}/>}
